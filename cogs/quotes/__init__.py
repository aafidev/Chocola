...
class QuotesCog(commands.Cog):
    ...
    async def process_quote_reaction(self, message, moyai_reaction):
        ...
        if quotes_channel:
            if is_text_embed:
                ...
                embed = nextcord.Embed(
                    title=f"Quote by {quote_author}",
                    description=quote_content,
                    color=nextcord.Color.blue(),
                )
                embed.set_footer(
                    text=f"Quoted at {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} UTC"
                )
        ...
        await self.log_quote(
            quote_content, quote_author, datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
        )
    ...
    async def on_ready(self):
        ...
        async with self.bot.db.execute(
            "SELECT guild_id, channel_id FROM channels"
        ) as cursor:
            async for row in cursor:
                ...
                if guild:
                    ...
                    if channel:
                        ...
                        async with self.bot.db.execute(
                            "SELECT message_id FROM reactions WHERE channel_id = ?",
                            (channel_id,),
                        ) as msg_cursor:
                            messages_in_db = {
                                row[0] for row in await msg_cursor.fetchall()
                            }
                            ...
                            async for old_message in channel.history(
                                ...
                                if old_message.id not in messages_in_db:
                                    moyai_reaction = next(
                                        (
                                            react
                                            for react in old_message.reactions
                                            if str(react.emoji) == self.moyai_emoji
                                        ),
                                        None,
                                    )
                                    if (
                                        moyai_reaction
                                        and moyai_reaction.count
                                        >= self.create_channel_threshold
                                    ):
                                        # Process the reaction as before
                                        await self.process_quote_reaction(
                                            old_message, moyai_reaction
                                        )

                                        # Update the 'reactions' table in the database
                                        await self.bot.db.execute(
                                            "INSERT OR IGNORE INTO reactions (channel_id, message_id) VALUES (?, ?)",
    ...
    async def on_reaction_add(self, reaction, user):
        if reaction.emoji == self.moyai_emoji:
            print(
                f"Reaction added: {reaction.emoji} by {user.name} in {reaction.message.channel.name}"
            )
            ...
            quotes_channel = nextcord.utils.get(
                reaction.message.guild.channels, name="quotes"
            )
            if (
                not quotes_channel
                and moyai_reaction
                and moyai_reaction.count >= self.create_channel_threshold
            ):
                quotes_channel = await reaction.message.guild.create_text_channel(
                    name="quotes"
                )
                ...
                print(
                    f"Created channel {quotes_channel.name} in {reaction.message.guild.name}"
                )

                # Once the "quotes" channel is created, send an embed to the channel
                embed = nextcord.Embed(
                    title="Welcome to the Quotes Channel!",
    ...
    @nextcord.slash_command(
        name="setthreshold",
        description="""Set the threshold for creating the "quotes" channel.
    ...
    async def set_threshold(self, ctx, threshold: int):
        ...
        await ctx.send(
            f'Threshold set to {self.create_channel_threshold} reactions for creating the "quotes" channel.'
        )

    @nextcord.slash_command(
        name="quotescanall",
        description="""Scan all messages in all channels and save quotes to the log.""",
    )
            ...
            for channel in guild.channels:
                if isinstance(channel, nextcord.TextChannel):
                    async for message in channel.history(limit=None):
                        if self.moyai_emoji in [
                            str(react.emoji) for react in message.reactions
                        ]:
